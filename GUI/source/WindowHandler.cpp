...
namespace HomeworkHelper
{
    ...
    int WindowHandler::CreateWindow(const int aHeight, const int aWidth, const std::string& aTitle)
    {
        auto& instance = GetInstance();
    ...
    void WindowHandler::UpdateWindow(Window& outWindow)
    {
        ...
        if (outWindow.show_demo_window)
            ImGui::ShowDemoWindow(&outWindow.show_demo_window);
        ...
        // 2. Show a simple window that we create ourselves. We use a Begin/End pair to create a named window.
        {
            ...
            ImGui::Checkbox("Demo Window", &outWindow.show_demo_window); // Edit bools storing our window open/close state
            ImGui::Checkbox("Another Window", &outWindow.show_another_window);

            ImGui::SliderFloat("float", &f, 0.0f, 1.0f); // Edit 1 float using a slider from 0.0f to 1.0f
            ImGui::ColorEdit3("clear color", outWindow.clear_color);
        ...
        if (outWindow.show_another_window) {
            ImGui::Begin("Another Window", &outWindow.show_another_window);
            // Pass a pointer to our bool variable (the window will have a closing button that will clear the bool when clicked)
            ImGui::Text("Hello from another window!");
            if (ImGui::Button("Close Me"))
                outWindow.show_another_window = false;
            ImGui::End();
        }

        // Rendering
        ImGui::Render();
        outWindow.windowData->ClearValue.color.float32[0] = outWindow.clear_color[0] * outWindow.clear_color[3];
        outWindow.windowData->ClearValue.color.float32[1] = outWindow.clear_color[1] * outWindow.clear_color[3];
        outWindow.windowData->ClearValue.color.float32[2] = outWindow.clear_color[2] * outWindow.clear_color[3];
        outWindow.windowData->ClearValue.color.float32[3] = outWindow.clear_color[3];
